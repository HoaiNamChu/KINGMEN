                ...
                <div class="col-md-12 col-lg-4">
                    <div class="shipping-form-cart-totals">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            @if($order->payment_status == 'Chưa thanh toán' && $order->payment_method == 'VN PAY' && $order->status != 'Đã hủy' && $order->status != 'Hoàn đơn' && $order->status != 'Đơn yêu cầu hoàn trả' && $order->status != 'Không giao được')
                                <a class="btn-theme btn-flat" href="{{ route('payback', $order->id) }}">Thanh toán lại</a>
                            @endif
                            ...
                            @if(!in_array($order->status, ['Đang chờ xác nhận', 'Hoàn đơn', 'Đã hủy', 'Hoàn thành', 'Đơn yêu cầu hoàn trả', 'Không giao được' ,'Đã xác nhận']))
                            ...
                            @if($order->status === 'Hoàn thành' && $order->completed_at != null)
                                @if(now()->diffInDays($order->completed_at) <= 10)

                                    <button class="btn-theme btn-flat" data-bs-toggle="modal"
                                            data-bs-target="#exampleModal">Hoàn
                                        trả đơn
                                    </button>
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <form id="returnForm" action="{{ route('order.return', $order->id) }}"
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="submit" class="btn btn-primary">Gửi yêu cầu
                                                        </button>
                                                    </div>
                                                </form>
                                            </div>
                                ...
                                @else
                                    <p class="text-muted">Đơn hàng đã quá thời gian hoàn trả.</p>
                                @endif
                            @endif
